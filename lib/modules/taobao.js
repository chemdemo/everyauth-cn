...
oauthModule.submodule('taobao')
    .configurable({
        ...
        format: '需要返回的类型，可选值是xml和json，默认json'
    })
    ...
    .fetchOAuthUser( function (accessToken) {
        ...
        var format = this._format || 'json';
        url += '?access_token=taobao.user' + accessToken;
        url += '&method=' + (this._role || 'buyer') + '.get';console.log('taobao user: ', this._role)
        url += '&format=' + format;
        url += 'v=2.0';
        ...
        request.get(url, function(err, res, body) {
            ...
            if('json' === format) {
                ...
                return p.fail(new Error('The response format does not supported yet.'));
            }
        });
        return p;
    })
