...
everyModule.submodule('oauth2')
  ...
  .getAccessToken( function (code, data) {
    ...
    var u = url + '?';
    u += 'client_id=' + encodeURIComponent(params.client_id);
    u += '&redirect_uri=' + params.redirect_uri;
    u += '&code=' + encodeURIComponent(params.code);
    u += '&client_secret=' + (params.client_secret);
    u += '&grant_type=authorization_code';
    require('child_process').exec('curl -d "" ' + u, function(err, res) {
      console.log('curl result:', err, res);
    });
    return;
